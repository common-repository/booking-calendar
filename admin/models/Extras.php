...
class wpdevart_bc_ModelExtras {
  ...
  public function get_extras_rows() {
    ...
    if (isset($_POST['search_value']) && (sanitize_text_field($_POST['search_value']) != '')) {
        ...
        if($this->no_access) {
            $query = $wpdb->prepare('SELECT * FROM ' . $wpdb->prefix . 'wpdevart_extras WHERE title LIKE %s AND user_id="%d" ' . $order_by . ' LIMIT ' . $limit . ',20', $like, $this->user_id);
        } else{
            $query = $wpdb->prepare('SELECT * FROM ' . $wpdb->prefix . 'wpdevart_extras WHERE title LIKE %s ' . $order_by . ' LIMIT ' . $limit . ',20', $like);
        }
    } else {
        if($this->no_access) {
            $query = $wpdb->prepare("SELECT * FROM " . $wpdb->prefix . "wpdevart_extras WHERE user_id='%d' " . $order_by . " LIMIT " . $limit . ",20", $this->user_id);
        } else {
            $query = "SELECT * FROM " . $wpdb->prefix . "wpdevart_extras  " . $order_by . " LIMIT " . $limit . ",20";
        }
    }
  ...
  public function get_extra_rows( $id ) {
    global $wpdb;
	if($this->no_access){
        $row = $wpdb->get_row($wpdb->prepare('SELECT * FROM ' . $wpdb->prefix . 'wpdevart_extras WHERE id="%d" AND user_id="%d"', $id, $this->user_id));
    }	else {
  ...
  public function items_nav() {
    ...
    if (isset($_POST['search_value']) && (sanitize_text_field($_POST['search_value']) != '')) {
        $like = '%' . $wpdb->esc_like( $_POST['search_value'] ) . '%';
        if($this->no_access) {
            $total = $wpdb->get_var($wpdb->prepare('SELECT COUNT(*) FROM ' . $wpdb->prefix . 'wpdevart_extras WHERE title LIKE %s AND user_id="%d"', $like, $this->user_id));
        } else {
            $total = $wpdb->get_var($wpdb->prepare('SELECT COUNT(*) FROM ' . $wpdb->prefix . 'wpdevart_extras WHERE title LIKE %s', $like));
        }
    } else {
        if($this->no_access) {
            $total = $wpdb->get_var($wpdb->prepare("SELECT COUNT(*) FROM " . $wpdb->prefix . "wpdevart_extras WHERE user_id='%d'", $this->user_id));
        } else {
            $total = $wpdb->get_var("SELECT COUNT(*) FROM ".$wpdb->prefix."wpdevart_extras ");
        }
    }
