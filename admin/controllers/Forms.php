...
class wpdevart_bc_ControllerForms {
	...
	private function save( $id ){
		...
		global $wpdb;
		$textareas = array();
		foreach(array_slice($_POST, 1, -4) as $key => $value){
			if(isset($value["type"]) && $value["type"] == "select"){
				$textareas[] = $key;
			}
		}
		$saved_parametrs = wpdevart_bc_Library::sanitizeAllPost(array_slice($_POST, 1, -4),$textareas);
	...
	private function duplicate( $id ){
		if ( ! isset( $_GET['_wpdevart_bc_nonce'] ) || ! wp_verify_nonce( $_GET['_wpdevart_bc_nonce'], 'duplicate_item' ) ) {
		   die('Sorry, your nonce did not verify.');
		}
		global $wpdb; 
