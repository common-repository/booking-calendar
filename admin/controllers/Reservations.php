...
class wpdevart_bc_ControllerReservations {
	...
	private function delete( $id ){
		if ( ! isset( $_POST['_wpdevart_bc_nonce'] ) || ! wp_verify_nonce( $_POST['_wpdevart_bc_nonce'], 'action_item' ) ) {
	...
	private function mark_read_selected( $id ){
		...
		if(isset($_POST['check_for_action'])){
			...
			$update_res = $wpdb->query($wpdb->prepare('UPDATE '.$wpdb->prefix . 'wpdevart_reservations SET is_new=0 WHERE id IN (%s)', $check_ids));
		}
		$this->display_reservations(0,array());
	}
